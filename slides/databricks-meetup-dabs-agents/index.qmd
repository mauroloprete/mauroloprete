...
**Unity AI Gateway V2** (Beta): agrega guardrails LLM-based (jailbreak, hallucination, custom) via UI. Todavia no integrado con DABs ni Databricks Apps.

::: notes
En vez de apuntar la app directo al Foundation Model, creamos un Model Serving Endpoint como proxy con AI Gateway. Rate limit de 30 llamadas por minuto por usuario. Guardrails de input: safety para contenido peligroso y PII BLOCK para bloquear tarjetas de crédito y datos personales antes de que lleguen al LLM. La inference table loguea todas las llamadas a Delta para monitoring. Output guardrails no soporta streaming. Unity AI Gateway V2 está en Beta desde junio 2026: agrega guardrails evaluados por LLM (jailbreak, hallucination, custom prompts) pero se configuran solo desde la UI por ahora — no hay recurso DABs para endpoints V2 y el sistema de permisos no se integra con el SP de las Databricks Apps todavía. Para producción con Apps, por ahora usar el ai_gateway declarativo sobre el serving endpoint.
:::
...
3. *"Haceme un cheesecake"* → Fuera de contexto, sin RAG
...
::: notes
Las primeras 2 preguntas muestran el RAG funcionando: el bot busca en el blog, arma contexto, y responde con fuentes. La 3 muestra que el system prompt funciona: como no hay contexto sobre cheesecakes en el blog, responde "no tengo información". La 4 muestra el AI Gateway en acción: manda un número de tarjeta de crédito y el gateway lo detecta y bloquea con PII BLOCK antes de que llegue al LLM. Hago la demo yo desde mi cuenta porque la app requiere auth de Databricks.
:::
...
| **Unity AI Gateway V2** | Guardrails LLM-based (jailbreak, hallucination), budgets, metrics | {{< fa flask >}} Beta |
