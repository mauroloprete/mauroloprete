...
::: notes
Esta es la arquitectura completa de Spark de Ideas Bot — mi réplica. 7 recursos, todos en un databricks.yml con engine direct. Arriba: el pipeline de conocimiento — un Job scrapea directamente el blog con BeautifulSoup, genera chunks y los escribe a Delta. Vector Search indexa automáticamente. En el medio: la comunidad usa la Databricks App (Streamlit), que llama al Serving Endpoint con AI Gateway. Abajo: Lakebase con PostgreSQL 17 para memoria conversacional — cada usuario tiene su thread_id y la memoria persiste entre sesiones. Todo deployable con un solo comando.
:::
...
ggplot() +
    ...
    annotate("rect", xmin = 0, xmax = 2.5, ymin = 2.8, ymax = 3.8,
             fill = "#64748b", color = "#475569", linewidth = 0.4) +
    annotate("text", x = 1.25, y = 3.5, label = "Blog (HTML)",
             color = "white", size = 2.3, fontface = "bold") +
    annotate("text", x = 1.25, y = 3.15, label = "Scraping directo",
             color = "#cbd5e1", size = 1.8) +
    annotate("text", x = 1.25, y = 2.95, label = "del sitio publico",
             color = "#cbd5e1", size = 1.8) +
    ...
    annotate("text", x = 4.75, y = 3.52, label = "Load KB Job",
             color = "white", size = 2.3, fontface = "bold") +
    annotate("text", x = 4.75, y = 3.2, label = "BeautifulSoup",
             color = "#c4b5fd", size = 1.8) +
    annotate("text", x = 4.75, y = 3.0, label = "chunks + metadata",
             color = "#c4b5fd", size = 1.8) +
    ...
    # Blog posts list (bottom left)
    annotate("rect", xmin = 0.2, xmax = 3.8, ymin = 0.3, ymax = 2.2,
             fill = "white", color = "#e2e8f0", linewidth = 0.3) +
    annotate("text", x = 2.0, y = 2.0, label = "Posts scrapeados",
             color = "#1e293b", size = 2.2, fontface = "bold") +
    annotate("text", x = 2.0, y = 1.65, label = "Tips #2: Delta Lake",
             color = "#64748b", size = 1.8, hjust = 0.5) +
    annotate("text", x = 2.0, y = 1.4, label = "Tips #3: Unity Catalog",
             color = "#64748b", size = 1.8, hjust = 0.5) +
    annotate("text", x = 2.0, y = 1.15, label = "Tips #8: Jobs & Workflows",
             color = "#64748b", size = 1.8, hjust = 0.5) +
    annotate("text", x = 2.0, y = 0.9, label = "Data Contracts Framework",
             color = "#64748b", size = 1.8, hjust = 0.5) +
    annotate("text", x = 2.0, y = 0.65, label = paste0("... ", length(list.files("../../blog/posts")), " posts"),
...
::: notes
Así se ingesta la base de conocimiento. Un Job de Databricks scrapea directamente el blog con requests y BeautifulSoup — va a la URL pública, extrae las URLs de todos los posts, descarga cada uno, y genera chunks de ~2000 caracteres con título, categoría y fuente. Se escriben a una tabla Delta en Unity Catalog. Vector Search sincroniza automáticamente esa tabla y genera el índice con embeddings. Si mañana publico un post nuevo, el job lo levanta solo en el próximo run. Todo declarado en el bundle.
:::
