...
The experiment is simple: a 5-million-row table in a Postgres, one query that gets pushed down entirely, one that doesn't, and each one's plan to see the difference. The Postgres comes from [Neon](https://neon.com), which gives you a serverless one with a public endpoint, no credit card asked. Everything is at [spark-de-ideas-labs/tips/query-federation](https://github.com/mauroloprete/spark-de-ideas-labs/tree/main/tips/query-federation): the step-by-step to create the database via UI or CLI, the SQL, and the reference outputs.
