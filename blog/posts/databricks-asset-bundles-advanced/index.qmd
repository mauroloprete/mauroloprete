...
```{#lst-complex-var .yaml lst-cap="Variable complex: configuración de cluster reutilizable en DABs"}
variables:
  cluster_config:
...
```{#lst-lookups .yaml lst-cap="Lookups: resolver IDs de recursos por nombre en deploy time"}
variables:
  shared_cluster:
...
```{#lst-artifacts-whl .yaml lst-cap="Artifacts: empaquetar wheels con versión dinámica"}
bundle:
...
```{#lst-monorepo-structure lst-cap="Estructura de monorepo con un bundle por dominio"}
project/
...
```{#lst-bundle-root .yaml lst-cap="Bundle raíz: includes, variables y targets por ambiente"}
# databricks.yml
bundle:
  name: data-platform
...
```{#lst-shared-cluster .yaml lst-cap="Cluster compartido con autoscale definido como variable complex"}
# shared/clusters.yml
variables:
  shared_cluster:
    type: complex
...
```{#lst-domain-resource .yaml lst-cap="Recurso de dominio: job de ingesta con schedule y cluster compartido"}
...
```{#lst-presets .yaml lst-cap="Presets: control granular de prefijos, triggers y tags por target"}
targets:
  dev:
    mode: development
    presets:
...
```{#lst-run-as-prod .yaml lst-cap="Patrón de producción: run_as con service principal y permisos"}
targets:
  prod:
    mode: production
    run_as:
...
```{#lst-cicd-github .yaml lst-cap="CI/CD con GitHub Actions: validar y deployar DABs automáticamente"}
...
```{#lst-sync-patterns .yaml lst-cap="Sync patterns: controlar qué archivos se suben al workspace"}
sync:
  include:
    - "src/**/*.py"
    - "config/**/*.yml"
...
```{#lst-bundle-var-env .bash lst-cap="Inyectar variables de entorno BUNDLE_VAR en deploy de CI/CD"}
...
- [Laboratorio práctico — bundle + CLI](https://github.com/mauroloprete/spark-de-ideas-labs/tree/main/tips/databricks-asset-bundles-advanced) — para ejecutar en Databricks Free Edition
