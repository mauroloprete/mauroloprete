...
```{#lst-training-skew .python lst-cap="Training-serving skew: data leakage por ventana temporal"}
...
```{#lst-feature-table .python lst-cap="Crear Feature Table con timestamp key para point-in-time"}
...
```{#lst-pit-lookup .python lst-cap="Point-in-time lookup: evitar data leakage en training set"}
...
```{#lst-pipeline-features .python lst-cap="Pipeline de features: batch diario + streaming en DLT"}
...
```{#lst-online-store .python lst-cap="Publicar features a Online Store para serving en milisegundos"}
from databricks.feature_engineering import FeatureEngineeringClient
...
```{#lst-freshness-check .python lst-cap="Monitoreo de freshness: alertar si los features son stale"}
