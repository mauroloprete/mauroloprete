...
```{#lst-namespace-3lvl lst-cap="Namespace de 3 niveles en Unity Catalog"}
metastore
...
```{#lst-catalogs-by-env .sql lst-cap="Opcion A: catalogs organizados por ambiente (dev, staging, prod)"}
CREATE CATALOG dev;
...
```{#lst-catalogs-by-domain .sql lst-cap="Opcion B: catalogs organizados por dominio de negocio"}
...
```{#lst-grants-schema .sql lst-cap="GRANTS heredados: error comun vs. patron correcto a nivel schema"}
...
```{#lst-row-col-security .sql lst-cap="Row y column level security: masking de PII y filtros por region"}
...
```{#lst-lineage .sql lst-cap="Linaje automatico: buenas practicas para trazabilidad completa"}
...
```{#lst-external-location .sql lst-cap="External Location y tabla externa para habilitar linaje en S3"}
...
```{#lst-volumes .sql lst-cap="Volumes: gestionar archivos no tabulares con gobernanza UC"}
