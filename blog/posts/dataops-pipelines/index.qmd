...
```{#lst-cicd-gha .yaml lst-cap="CI/CD para pipelines con GitHub Actions y Databricks Asset Bundles"}
...
```{#lst-tests-quality .python lst-cap="Tests automatizados de duplicados y freshness en PySpark"}
...
```{#lst-dbt-tests .yaml lst-cap="Validación de schema y calidad con tests de dbt"}
...
```{#lst-monitoring .python lst-cap="Monitoreo básico post-ejecución: volumen y nulidad por columna"}
import logging
