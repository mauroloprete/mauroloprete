...
```{#lst-trigger-modes .python lst-cap="Trigger modes: processingTime vs availableNow según latencia"}
...
```{#lst-watermark .python lst-cap="Watermark: limitar el estado en memoria para aggregations"}
...
```{#lst-auto-loader .python lst-cap="Auto Loader vs file source: schema evolution y exactly-once"}
...
```{#lst-rescue-column .python lst-cap="Rescue column: capturar datos que no matchean el schema"}
...
```{#lst-foreach-batch .python lst-cap="foreachBatch con MERGE: upsert idempotente en streaming"}
...
```{#lst-stream-monitor .python lst-cap="StreamingQueryListener: monitorear batches y detectar lag"}
