...
```{#lst-contract-yaml .yaml lst-cap="Definición YAML de un Data Contract con schema, SLAs y reglas de calidad"}
...
```{#lst-parser .python lst-cap="Parser de contratos YAML a dataclasses de Python"}
import yaml
...
```{#lst-schema-validator .python lst-cap="Validador de schema: compara DataFrame contra el contrato"}
...
```{#lst-quality-validator .python lst-cap="Validador de calidad: ejecuta reglas SQL y reporta violaciones"}
...
```{#lst-ingest-pipeline .python lst-cap="Pipeline de ingesta con validación de contrato integrada"}
...
```{#lst-hard-vs-soft .python lst-cap="Hard contract vs Soft contract: fallar o enviar a cuarentena"}
...
```{#lst-dlt-expectations .python lst-cap="Data Contracts como DLT Expectations en Databricks"}
import dlt
