...
-   **Lakebase** (Postgres serverless) escribe directamente en Delta e Iceberg. Sin ETL, sin réplicas.
-   Elimina tres categorías de infra: **CDC/ETL** de ida, **Reverse ETL** de vuelta, y **serving layers** de cache.
...
La idea central: los datos transaccionales se escriben **directamente en Delta e Iceberg desde el punto de escritura**. No después, no con un CDC de por medio, no con un job batch nocturno. Desde el momento cero.
...
>
> --- Ali Ghodsi, CEO de Databricks — DAIS 2026 Keynote
...
|:---|:---|:---|
| **Lakebase** | Motor transaccional | Postgres serverless sobre object storage abierto. 12M launches/día. |
...
| **Lakehouse//RT** | Serving real-time | Queries sub-milisegundo sobre datos gobernados en Delta/Iceberg. |
...
La diferencia clave: HTAP intentó meter todo en un motor y nadie lo usó en producción seria porque un query analítico pesado te mataba las transacciones. Zero ETL (como el de AWS entre Aurora y Redshift) automatiza la copia, pero seguís teniendo dos copias con latencia entre ellas.
...
flowchart TD
    ...
    RT("<b>Lakehouse//RT</b><br><small>Serving sub-ms</small>")
    ...
    LB -- "escribe directo en Delta / Iceberg" --> UC
    UC --> DBT2 --> DASH2
    UC --> RT
    UC --> APPS
    UC --> AGENTS
...
-   **No hay Debezium**. No hay CDC. Lakebase escribe nativamente en Delta/Iceberg.
-   **No hay Kafka**. No hay sistema de mensajería intermedio.
-   **No hay Structured Streaming** para ingestar. El dato ya está en el lake.
-   **No hay Reverse ETL**. La app y el warehouse leen del mismo storage. Si tu modelo Gold calcula un score de churn y querés usarlo en la app, no necesitás empujarlo de vuelta — ya está ahí, accesible desde Lakebase.
-   **No hay serving layer separada**. Lakehouse//RT resuelve queries sub-milisegundo directamente sobre los datos gobernados. No necesitás Redis ni Pinot como cache intermedio.
-   **Latencia cero** entre la escritura operacional y la disponibilidad analítica. Es el mismo dato.
...
-   Con Lakebase Search, el retrieval (vector + full-text) también corre en el mismo backend
...
::: callout-note
Esto explica por qué Databricks está empujando LTAP tan fuerte en el contexto de su plataforma agentic. Si tenés Genie, Agent Bricks, o agentes custom corriendo en producción, cada milisegundo de latencia y cada pipeline intermedio es fricción. LTAP elimina esa fricción de raíz.
:::
...
Además de LTAP, Lakebase trajo features propios que vale la pena conocer:
...
El loop completo de un agente (retrieve → reason → act → remember) puede correr sobre un solo backend. Te ahorrás montar un Pinecone o Weaviate al lado.

------------------------------------------------------------------------

## Quién ya lo usa

Lakebase (la base de LTAP) ya está en producción con clientes grandes:
...
Si LTAP cumple, hay una cantidad significativa de infraestructura que va a dejar de tener razón de ser. Debezium, Kafka Connect para CDC, Structured Streaming como paso de ingesta, las tablas Bronze que son copias crudas de la fuente, las herramientas de Reverse ETL, las serving layers de cache — todo eso se simplifica drásticamente.
